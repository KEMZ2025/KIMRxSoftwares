...
<head>
    ...
    <title>Cancelled Sales - KIM Rx</title>
    <style>
        * { box-sizing: border-box; }
        ...
        .filters {
            ...
            padding: 16px;
            border-radius: 12px;
            background: linear-gradient(135deg, #fff8f6 0%, #fdf3f1 100%);
    ...
    <div class="layout">
        ...
        <div class="content" id="mainContent">
            ...
            <div class="panel">
                ...
                @if(session('success'))
                    <div style="background:#e7f6ec; color:#1f7a4f; padding:12px; border-radius:8px; margin-bottom:15px;">
                        {{ session('success') }}
                    </div>
                @endif

                <form method="GET" action="{{ route('sales.cancelled') }}">
                    <div class="filters">
                        <div class="field">
                        ...
                        <div class="filter-actions">
                            ...
                            <a href="{{ route('sales.cancelled', ['clear_filters' => 1]) }}" class="btn btn-reset">Reset</a>
                        </div>
                    </div>
                </form>
                        ...
                        <tbody>
                            @forelse($sales as $sale)
                                <tr>
                                    ...
                                    <td>
                                        @if($sale->cancelled_from_status === 'approved')
                                            <span class="badge badge-approved">Approved</span>
                                        @else
                                            <span class="badge badge-pending">{{ ucfirst($sale->cancelled_from_status ?: 'pending') }}</span>
                                        @endif
                                    </td>
                                    <td>{{ $sale->cancelled_at?->format('d M Y H:i') ?? 'N/A' }}</td>
                                    <td>{{ $sale->cancelledByUser?->name ?? 'N/A' }}</td>
                                    <td>{{ number_format((float) $sale->total_amount, 2) }}</td>
                                    <td class="reason-cell">{{ $sale->cancel_reason ?: 'No reason captured.' }}</td>
                                    <td>
                                        <a href="{{ route('sales.show', array_merge(['sale' => $sale->id, 'return_to' => 'sales.cancelled'], $filters, request()->filled('page') ? ['page' => request('page')] : [])) }}" class="btn btn-open">Open</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
